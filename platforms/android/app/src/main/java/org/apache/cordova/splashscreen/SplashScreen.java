    ...
    private void showSplashScreen(final boolean hideAfterDelay) {
        ...
        cordova.getActivity().runOnUiThread(new Runnable() {
            public void run() {
                ...
                splashDialog.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN); // 解除强制全屏模式
                splashDialog.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS); // 添加半透明状态栏
                // window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUND);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    splashDialog.getWindow().getDecorView().setFitsSystemWindows(true);
                }
